...
static esp_err_t root_get_handler(httpd_req_t *req) {
    const char html[] =
        ...
        "<script src=\"https://cdn.jsdelivr.net/npm/sam-js\"></script>"
        "<script>"
        ...
        "if(!t)return;"
        "try{"
        "if(!window.SamJs) throw new Error('SAM JS not loaded');"
        "let sam=new SamJs({pitch:64, speed:72, mouth:128, throat:128});"
        "let wav=sam.wav(t);"
        "let ratio=22050/16000;"
        "let out_len=Math.floor((wav.length-44)/ratio);"
        "let out=new Int16Array(out_len);"
        "for(let i=0;i<out_len;i++){"
        "let in_idx=44+Math.floor(i*ratio);"
        "out[i]=(wav[in_idx]-128)*256;"
        "}"
        "fetch('/audio',{method:'POST',body:new Blob([out.buffer])});"
